...
const messages = {
  ...
  answerMessage: `<div class="attempt__message attempt__message--answer">${answerMessage}</div>`,
  ...
  compare: `<div class="attempt__message">Batna will compare your offer to offers of other customers. If your offer is high enough — you will get an approval!</div>`,
  ...
  offerPrice: `<div class="attempt__message attempt__message--first">Congratulations! We have a personal offer for you!</div>
...
function addMessage (html: string) {
  const content = document.querySelector('.attempt__content');
  if (!content) return;
  content.innerHTML += html;
}

function addTitle (title: string, price: string, image: string) {
  if (isFirstLoading) {
...
setTimeout(async () => {
  ...
  if (addToCartButton) {
    ...
    const response = await fetch(`${currentHref}/metafields.js`).then(res => res.json());
    const metafields = await response;
    ...
    showButton.addEventListener('click', async (e) => {
      ...
      if (search && metafields.variants) {
        ...
        const variant = metafields.variants.find((item: { id: string; }) => item.id == id)
        variant ? price = variant.price : price
      }

      if (isFirstLoading || !isGreeting) {
